...
public class RestClientConfig {
  private final KisCredentialsProperties kisCredentialsProperties;
  ...
  public RestClientConfig(
      KisCredentialsProperties kisCredentialsProperties,
      KisRateLimiter rateLimiter,
      KisTokenManager tokenManager
  ) {
    this.kisCredentialsProperties = kisCredentialsProperties;
  ...
  private RestClient buildKisRestClient(boolean blocking) {
    return RestClient.builder()
        ...
        .requestInterceptor((request, body, execution) -> {
          Credential credential = selectCredential(blocking);
  ...
  private Credential selectCredential(boolean blocking) {
    List<Credential> candidates = kisCredentialsProperties.getValidCredentials();
    if (candidates.isEmpty()) {
      throw new IllegalStateException("No valid KIS credentials configured.");
    }
    ...
    if (blocking) {
      Credential credential = candidates.get(start);
      rateLimiter.acquire(credential.appKey());
      return credential;
    }

    for (int i = 0; i < size; i++) {
      Credential credential = candidates.get((start + i) % size);
      if (rateLimiter.tryAcquire(credential.appKey())) {
        return credential;
      }
    }

    throw new IllegalStateException("All KIS credentials are rate-limited.");
  }
